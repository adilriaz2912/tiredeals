        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="card p-6">
            ...
            <div className="mt-4">
              ...
              <p className="text-sm text-slate-400 mt-1">Avg YoY Growth</p>
            </div>
          </div>
          ...
          <div className="card p-6">
            ...
            <div className="mt-4">
              ...
              <p className="text-sm text-slate-400 mt-1">Top Performing Region</p>
            </div>
          </div>
        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className={`text-center mt-2 font-bold ${
                        metric === 'growth'
                          ? region.growth >= 15 ? 'text-emerald-400' : 'text-cyan-400'
                          : 'text-emerald-400'
                      }`}>
                      ...
                      <div className="flex flex-wrap gap-1 justify-center mt-2">
                        {region.states.slice(0, 3).map(state => (
                          <span key={state} className="text-xs text-slate-500">{state}</span>
                        ))}
                        {region.states.length > 3 && (
                          <span className="text-xs text-slate-500">+{region.states.length - 3}</span>
                        )}
                      </div>
                    </button>
                  )
                    ...
                    return (
                      <>
                        ...
                        <div className="grid grid-cols-4 gap-4 mb-4">
                          ...
                          <div>
                            <p className="text-sm text-slate-400">Installers</p>
                            <p className="text-lg font-semibold text-white">{region.installers}</p>
                          </div>
                        </div>
                        <div>
                          <p className="text-sm text-slate-400 mb-2">Top Products</p>
                          <div className="flex gap-2">
                            {region.topProducts.map(product => (
                              <span key={product} className="px-3 py-1 bg-slate-700 text-slate-300 text-sm rounded-lg">
                                {product}
                              </span>
                            ))}
                          </div>
          ...
          <div className="card">
            ...
            <div className="p-4 space-y-3">
              {topCities.map((city, i) => (
                <div key={city.city} className="p-3 bg-slate-800/40 rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                        i < 3 ? 'bg-gradient-to-br from-amber-500 to-orange-500 text-white' : 'bg-slate-700 text-slate-400'
                      }`}>
                        {i + 1}
                      </span>
                    ...
                    <span className={`flex items-center gap-1 text-sm ${
                      city.growth >= 20 ? 'text-emerald-400' : 'text-cyan-400'
                    }`}>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-slate-400">{city.orders} orders</span>
                  </div>
                </div>
              ))}
            </div>
