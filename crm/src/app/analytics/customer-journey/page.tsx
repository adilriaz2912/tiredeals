        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="card p-6">
            ...
            <div className="mt-4">
              ...
              <p className="text-sm text-slate-400 mt-1">Avg Time to Convert</p>
            </div>
          </div>
          ...
          <div className="card p-6">
            ...
            <div className="mt-4">
              ...
              <p className="text-sm text-slate-400 mt-1">Cart Abandonment</p>
            </div>
          </div>
        </div>
              ...
              {funnelStages.map((stage, i) => {
                ...
                return (
                  <div key={stage.stage} className="flex-1 flex flex-col items-center">
                    <div
                      className="w-full bg-gradient-to-r from-emerald-500 to-cyan-500 rounded-t-lg flex items-center justify-center py-8 relative"
                    ...
                    <div className="mt-4 text-center">
                      ...
                      <p className="text-sm text-slate-400">{stage.percentage}%</p>
                    </div>
          ...
          <div className="card">
            ...
            <div className="p-4 space-y-4">
              {dropOffPoints.map((point) => (
                <div key={point.from} className="p-4 bg-slate-800/40 rounded-xl">
                  ...
                  <p className="text-sm text-slate-400 mb-2">{point.reason}</p>
              ...
              {segments.map((segment) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-sm text-slate-400">{segment.percentage}% of visitors</span>
                  </div>
                  <div className="grid grid-cols-4 gap-4 text-center">
                    ...
                    <div>
                      <p className="text-sm text-slate-400">Touchpoints</p>
                      <p className="font-semibold text-white">{segment.touchpoints}</p>
                    </div>
                    <div>
                      <p className="text-sm text-slate-400">Conv Rate</p>
                      <p className="font-semibold text-emerald-400">{segment.conversion}%</p>
                    </div>
                    <div>
                      <p className="text-sm text-slate-400">Avg Value</p>
                      <p className="font-semibold text-cyan-400">{formatCurrency(segment.value)}</p>
                    </div>
                  </div>
                </button>
              ))}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-800/50">
                  <th className="text-left p-4 text-sm font-medium text-slate-400">Touchpoint</th>
                  <th className="text-right p-4 text-sm font-medium text-slate-400">Visits</th>
                  <th className="text-right p-4 text-sm font-medium text-slate-400">Conversions</th>
                  <th className="text-right p-4 text-sm font-medium text-slate-400">Conv Rate</th>
              ...
              <tbody>
                {touchpoints.map((tp) => {
                  ...
                  return (
                    <tr key={tp.touchpoint} className="border-b border-slate-800/30 hover:bg-slate-800/30">
                      <td className="p-4 font-medium text-white">{tp.touchpoint}</td>
                      <td className="p-4 text-right text-slate-300">{tp.visits.toLocaleString()}</td>
                      <td className="p-4 text-right text-slate-300">{tp.conversions.toLocaleString()}</td>
                      <td className="p-4 text-right">
                        <span className={`font-semibold ${
                          tp.rate >= 10 ? 'text-emerald-400' :
                          tp.rate >= 5 ? 'text-cyan-400' :
                          'text-amber-400'
                        }`}>
                          {tp.rate}%
                        </span>
