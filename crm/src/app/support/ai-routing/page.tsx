          ...
          <div className="card p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-400">Tickets Today</p>
              </div>
            </div>
          </div>
          ...
          <div className="card p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-400">Auto-Routed</p>
              </div>
            </div>
          </div>
          ...
          <div className="card p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-400">Auto-Resolved</p>
              </div>
            </div>
          </div>
          ...
          <div className="card p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-400">AI Accuracy</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 card">
            ...
            <div className="divide-y divide-neutral-800/30">
              {filteredTickets.map((ticket) => (
                <div key={ticket.id} className="p-6 hover:bg-neutral-800/20 transition-colors">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono text-sm text-neutral-400">{ticket.id}</span>
                  ...
                  {/* AI Analysis */}
                  <div className="p-4 bg-gradient-to-r from-green-400/5 to-green-500/5 border border-green-400/20 rounded-xl">
                    <div className="flex items-center gap-2 mb-3">
                      <Bot className="w-4 h-4 text-green-300" />
                      <span className="text-sm font-medium text-green-300">AI Analysis</span>
                      <span className="text-xs text-neutral-500">• {ticket.aiAnalysis.confidence}% confidence</span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                      ...
                      <div>
                        <p className="text-xs text-neutral-500">Suggested Agent</p>
                        <p className="text-sm text-green-400">{ticket.aiAnalysis.suggestedAgent}</p>
                      </div>
                    </div>

                    <div className="flex flex-wrap gap-2 mb-3">
                      {ticket.aiAnalysis.keyEntities.map((entity, i) => (
                        <span key={i} className="text-xs px-2 py-1 bg-neutral-800/60 text-neutral-300 rounded">
                          {entity}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="p-3 bg-neutral-800/40 rounded-lg">
                      <p className="text-xs text-neutral-500 mb-1">Suggested Response</p>
                      <p className="text-sm text-neutral-300">{ticket.aiAnalysis.suggestedResponse}</p>
                    </div>

                    {ticket.status === 'pending_assignment' && (
                      <div className="flex gap-2 mt-4">
                        <button className="flex-1 py-2 bg-green-500/20 text-green-400 rounded-lg text-sm font-medium hover:bg-green-500/30 transition-colors">
                          Accept Suggestion
                        </button>
                        <button className="flex-1 py-2 bg-neutral-700 text-neutral-300 rounded-lg text-sm font-medium hover:bg-neutral-600 transition-colors">
                          Manual Assign
                        </button>
                      </div>
                    )}
            ...
            <div className="card">
              ...
              <div className="p-4 space-y-3">
                {routingRules.map((rule) => (
                  <div key={rule.category} className="p-3 bg-neutral-800/40 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-medium text-white">{rule.category}</span>
                      {rule.autoAssign ? (
                        <span className="text-xs px-2 py-0.5 bg-green-500/20 text-green-400 rounded">Auto</span>
                      ) : (
                        <span className="text-xs px-2 py-0.5 bg-amber-500/20 text-amber-400 rounded">Manual</span>
                      )}
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-neutral-400">{rule.agent}</span>
                      <span className="text-neutral-500">Escalate: {rule.escalateAfter}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-neutral-400">Routing Accuracy</span>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-neutral-400">Priority Accuracy</span>
                    <span className="text-green-300">91.8%</span>
                  </div>
                  <div className="h-2 bg-neutral-800 rounded-full overflow-hidden">
                    <div className="h-full bg-green-400" style={{ width: '91.8%' }} />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-neutral-400">Auto-Resolution Rate</span>
                    <span className="text-green-400">16.7%</span>
                  </div>
                  <div className="h-2 bg-neutral-800 rounded-full overflow-hidden">
                    <div className="h-full bg-green-500" style={{ width: '16.7%' }} />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-neutral-400">Agent Satisfaction</span>
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    auto_assigned: 'bg-green-400/20 text-green-300',
    auto_resolved: 'bg-green-500/20 text-green-400',
  }
